...
router.post("/posts", (req, res) => {
  ...
  if (!(title && content)) {
    ...
    let file = req.files;
    if (
      ![".png", ".jpg", ".gif", ".jpeg"].includes(path.extname(file.img.name))
    ) {
      errors.push({ msg: "Votre image n'est pas au bon format." });
    } else if (req.files.img.size > 2000000) {
      errors.push({
        msg: "Votre image est trop grosse , la taille maximale est 2mo.",
      });
    } else {
      ...
      file.mv(`./user-data/post-images/${image}`, (err) => {
        if (err) {
          errors.push({
            msg: "L'image n'a pas été envoyée, veuillez réessayer.",
          });
        } else {
          ...
          newPost = new Posts({
            title,
            content,
            image: newPost._id + path.extname(file.name),
          });
          newPost.save();
